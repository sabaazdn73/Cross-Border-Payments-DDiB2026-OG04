...
export default function HashDisplay({ label, hash, className = '' }) {
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      {label && <p className="text-xs font-medium text-white/50 uppercase tracking-wider">{label}</p>}
      <div className="flex items-start gap-2">
        <code className="hash-display flex-1 text-xs leading-relaxed">{hash}</code>
        <button onClick={handleCopy} className="flex-shrink-0 p-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/50 hover:text-white transition-colors"
