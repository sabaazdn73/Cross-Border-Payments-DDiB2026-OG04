export default function PageHeader({ badge, title, subtitle, children, className = '' }) {
  return (
    <div className={`text-center mb-10 ${className}`}>
      ...
      {title && <h1 className="text-3xl md:text-4xl font-bold text-white mb-3">{title}</h1>}
      {subtitle && <p className="text-white/60 text-base max-w-2xl mx-auto leading-relaxed">{subtitle}</p>}
      {children}
    </div>
  );
}
