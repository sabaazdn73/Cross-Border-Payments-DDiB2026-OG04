...
The walletless design means end-users never create Hedera accounts, which is the product's core promise. Account growth therefore comes from the **operator tier**, not the consumer tier.
...
**Year 1 realistic new accounts: 5 to 15** (operator + 3 corridor partners + initial B2B customers). These are not consumer signups, but they are real, active accounts with recurring transaction volume, a higher-quality metric than dormant accounts.
...
|---|---|
| HCS finality | ~3s deterministic, so the compliance timestamp is legally meaningful because it is not probabilistic |
| Per-message fee | Fixed USD-denominated fractions of a cent, so the compliance anchor costs less than $0.001, so it is economically viable at all transfer sizes |
...
| TPS headroom | 10,000+ TPS available vs. our ~22 TPS at SOM, so no technical ceiling anywhere near this product's scale |
