...
A researched, named candidate list per region, not aspirational coverage claims. Verified as of mid-2026; this space moves monthly, re-check before relying on any specific detail.
...
| Candidate | Coverage | Notes |
|---|---|---|
| **Mercado Pago** | Consumer wallet with real money-transfer capability, not just card acquiring | Mexico, Brazil, Argentina, Colombia, Chile, Peru; the one multi-country player from common LatAm payments infographics that actually does wallet/transfer functions relevant here, not merchant card processing. |
| **EBANX** | Cross-border payment enabler for LatAm | Brazil-based, operating across multiple countries; its core business (connecting international merchants to local LatAm payment methods) is the closest direct analog to this project's own function in the region. |
| **PayU** | Global emerging-market payment processor with real cross-border reach | Colombia and broader LatAm, part of Prosus; genuine multi-country footprint, not a single-country processor. |
| **Bitso** | Consumer super-app, 9M users across the region | Exchange, stablecoin yield, cross-border payments; BVNK's own LatAm liquidity partner. |
| **Lemon Cash** (Argentina) / **Littio** (Colombia) | Regional neobanks | Serve the highest-inflation markets in the region with stablecoin yield products (7-9% and 5-8% APY respectively). |
| **dLocal Stablecoin** | Merchant-scale collection | Extends an existing payments network into stablecoin natively. Relevant if collection volume, not just payout, matters later. |

### Checked and deliberately excluded

A commonly-circulated LatAm payments infographic lists dozens of names per country (Getnet, Cielo, Rede, transbank, niubiz, izipay, klap, BciPagos, and many more). Checked against what this project actually needs: these are overwhelmingly **in-country merchant card acquirers and POS processors**, a genuinely different business function from cross-border settlement and payout. Listing them here would misrepresent what a real partnership with them would offer.
...
| Candidate | Relevance |
|---|---|
| **Circle / Circle Payments Network (CPN)** | Issues USDC directly; deepest liquidity and regulatory standing in the US and EU. Explicitly thinner in Africa, LatAm, and Southeast Asia by Circle's own positioning, a reason to pair with a regional specialist rather than rely on Circle alone outside its core markets. |
